...
from onixbind.openfold.model.primitives import Linear, LayerNorm, softmax_no_cast
from onixbind.openfold.model.dropout import DropoutRowwise
from onixbind.openfold.model.outer_product_mean import OuterProductMean
from onixbind.openfold.model.triangular_attention import (
    TriangleAttention,
)
from onixbind.openfold.model.triangular_multiplicative_update import (
    TriangleMultiplicationOutgoing,
    TriangleMultiplicationIncoming,
)
from onixbind.openfold.utils.chunk_utils import chunk_layer, ChunkSizeTuner
from onixbind.openfold.utils.tensor_utils import add,permute_final_dims,flatten_final_dims
from onixbind.openfold.model.primitives import Attention,_deepspeed_evo_attn
from onixbind.openfold.utils.atom_token_conversion import pad_at_dim,concat_previous_and_later_windows,slice_at_dim
...
class MSAPairWeightedAveraging(nn.Module):
    ...
    def forward(self, 
        ...
        # The released checkpoint's runtime hard-disables the DeepSpeed kernel
        # here (its own comment calls it unstable in this module), so the
        # PyTorch path is what the weights were validated against.  Leaving it
        # enabled would silently pick a different kernel than the reference
        # implementation and change the pair-weighted average.
        use_deepspeed_evo_attention = False
...
class MSABlock(nn.Module):
    ...
    def __init__(self,
        ...
        # In the final block the MSA representation is discarded right after
        # the outer-product-mean has written it into z, so the two sublayers
        # that only update m are omitted.  This is exact rather than an
        # approximation, and it is how the released checkpoint was trained:
        # it carries no weights for them.
        ...
        **kwargs,
        ...
        for block_index in range(no_blocks):
            block = MSABlock(
                ...
                inf=inf,
                eps=eps,
                is_last_block=(block_index == no_blocks - 1),
            )
            self.blocks.append(block)
