...
from onixbind.openfold.model.primitives import Linear, LayerNorm
from onixbind.openfold.model.pairformer import PairStack
from onixbind.openfold.model.diffusion import AtomAttentionEncoder, RelativePositionEncoding
import einops
from onixbind.openfold.utils.tensor_utils import add
...
class MSAEmbedder(nn.Module):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        # The released weights were validated with the query row pinned to
        # position 0.  Plain sampling lets it move, and drops it outright once
        # the alignment is deeper than msa_depth.
        self.preserve_query_row = bool(kwargs.get("preserve_query_row", True))

        self.linear_mf = Linear(c_msa_feat, c_m,bias=False)
        self.linear_s_inputs = Linear(c_s_inputs, c_m,bias=False)

    def select_rows(self, num_alignments, num_select, device=None):
        """Choose which alignment rows the model sees on this recycle.

        Kept as its own method so the choice is testable: with
        preserve_query_row the query row must come first and must never be
        dropped, which is what the released weights were fit on.
        """
        num_alignments = int(num_alignments)
        num_select = int(num_select)
        if self.preserve_query_row and num_alignments > 1 and num_select > 1:
            non_query = torch.randperm(num_alignments - 1, device=device)[:num_select - 1] + 1
            return torch.cat(
                [torch.zeros(1, dtype=torch.long, device=device), non_query], dim=0
            )
        if self.preserve_query_row:
            return torch.zeros(min(1, num_select), dtype=torch.long, device=device)
        return torch.randperm(num_alignments, device=device)[:num_select]
    ...
    def forward(self, batch,s_inputs,msa_mask,inplace_safe=False):
        ...
        random_indices = [
            self.select_rows(num_align, num_select, device=mf.device)
