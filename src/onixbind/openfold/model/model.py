...
import einops
from onixbind.openfold.model.backbone import BackboneTrunk
from onixbind.openfold.model.diffusion import DiffusionModule
from onixbind.openfold.model.heads import ConfidenceHead
from onixbind.openfold.model.affinity import AffinityModulePocket
from onixbind.openfold.utils.atom_token_conversion import aggregate_fn
...
class OnixBind(nn.Module):
    ...
    def __init__(self, config, generator = None):   
        ...
        """
        super(OnixBind, self).__init__()

        self.globals = config.globals
        self.sample_config = config.sample
        self.config = config
        self.confidence_enabled = bool(config.globals["confidence_enabled"])
        ...
        # One pocket-conditioned affinity head per ensemble member.  They share
        # every upstream tensor: the trunk and diffusion run once per record and
        # each head is evaluated on the same s_inputs / s / z / x_predicted.
        self.head_aliases = tuple(config.ensemble["members"])
        self.head_weights = tuple(float(w) for w in config.ensemble["weights"])
        if len(self.head_aliases) != len(self.head_weights):
            raise ValueError("ensemble members and weights must have equal length")
        self.affinity_heads = nn.ModuleList(
            [AffinityModulePocket(self.config) for _ in self.head_aliases]
        )
    ...
    def forward(self, input_features, diffusion_batch_size=1):
        ...
        # every head sees the same upstream tensors; the upstream is not rerun
        head_scores = {}
        for alias, head in zip(self.head_aliases, self.affinity_heads):
            head_out = head(
                backbone_outputs['s_inputs'],
                backbone_outputs['s'],
                backbone_outputs['z'],
                x_predicted,
                input_features,
            )
            head_scores[alias] = head_out['affinity_logits']
            # identical across heads: the pocket is defined by x_predicted
            outputs['pocket_fallback_code'] = head_out['pocket_fallback_code']
        outputs['head_affinity_logits'] = head_scores
        # the weighted mean is taken in float64 so the published value does not
        # depend on the autocast dtype the heads happened to run under
        stacked = torch.stack(
            [head_scores[a].double() for a in self.head_aliases], dim=0
        )
        weights = torch.tensor(
            self.head_weights, dtype=torch.float64, device=stacked.device
        ).reshape(-1, *([1] * (stacked.ndim - 1)))
        outputs['affinity_logits'] = (stacked * weights).sum(dim=0)
