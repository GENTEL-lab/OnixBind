...
try:
    ...
    # Build outside the package by default.  Building in place breaks a
    # read-only install and lets concurrent GPU workers race on the same
    # object files; ONIXBIND_LAYER_NORM_BUILD_DIR or TORCH_EXTENSIONS_DIR give
    # each run its own directory.
    build_directory = os.environ.get("ONIXBIND_LAYER_NORM_BUILD_DIR")
    if not build_directory:
        extensions_root = os.environ.get("TORCH_EXTENSIONS_DIR")
        if extensions_root:
            build_directory = os.path.join(extensions_root, "onixbind_layer_norm")
        else:
