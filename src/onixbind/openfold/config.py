...
def model_config(
    ...
    # The released weights were validated with attention run unchunked; the
    # reference runtime's config contract rejects any other value.  Chunking
    # changes the reduction order, so leaving the inherited default of 4 here
    # would quietly compute something the weights were never checked against.
    with c.unlocked():
...
config = mlc.ConfigDict(
    {
        ...
        "backbone": {
            ...
            "msa": {
                "msa_embedder": {
                    ...
                    "c_m": c_m,
                    "c_s_inputs": c_s_inputs,
                    # the reference runtime samples 2048 MSA rows; 1024 would
                    # silently truncate deeper alignments
                    "msa_depth": 2048,
                    "preserve_query_row": True,
                },
                "msa_stack": {
                    "c_m": c_m,
                    "c_z": c_z,
        ...
        "diffusion": {
            ...
            "diffusion_conditioning": {
                # the released weights normalise s_trunk / s_inputs and
                # z_trunk / relpos separately instead of concatenating raw
        ...
        "affinity_head": {
            # 8 A protein-ligand contact cutoff defines the pocket
            "cutoff": 8.0,
            # single pX logit per record
            "pocket_no_bins": 1,
            "_mask_trans": True,
            "c_z": c_z,
            "c_s": c_s,
            "c_s_inputs": c_s_inputs,
            # the affinity distogram is 2-22 A in 64 bins, unlike the
            # confidence head's 3.25-50.75 A in 39 bins
            "min_bin": 2,
            "max_bin": 22,
            "no_bins": 64,
            "max_num_atoms": 24,
        ...
        # e0 + e2.  The original pick was e0 + e3 on the reference's own
        # per-head predictions, but e3 turned out to reproduce poorly here: on
